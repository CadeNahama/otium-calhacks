...
from uagents import Model, Agent
from uagents.mailbox import MailboxClient
from .protocols import TaskMessage, CommandPlan, ExecutionRequest, ExecutionResult, CommandStepModel # Added CommandStepModel
# --- END NEW IMPORTS ---

# Import existing modules
# NOTE: Agent, SSHManager, CommandExecutor are now managed by the AEAs, 
# but are imported here for compatibility with other existing file logic (e.g., helper functions).
...
# --- NEW FETCH.AI AGENT CONFIGURATION ---
# The FastAPI now hosts a mailbox client to talk to the Orchestrator AEA.
ORCHESTRATOR_AEA_ADDRESS = os.getenv("ORCHESTRATOR_AEA_ADDRESS", "default_orchestrator_address")
MAILBOX_CLIENT_SEED = os.getenv("MAILBOX_CLIENT_SEED", "otium_fastapi_mailbox_seed")
MAILBOX_URL = os.getenv("MAILBOX_URL", "http://127.0.0.1:8000/mailbox") # Example: use local host 

# Initialize a dummy Agent instance for the Mailbox Client
fastapi_agent = Agent(name="FastAPIAgent", seed=MAILBOX_CLIENT_SEED)
mailbox_client = MailboxClient(
    agent=fastapi_agent, 
    inbox_url=MAILBOX_URL,
    # This requires an inbox to be set up, which is complex for Phase 1.
    # For this implementation, we will simulate a direct send/receive pattern 
    # that is common in uagents testing environments.
)
            ...
            if user_id in user_connections:
                for conn_id in list(user_connections[user_id].keys()):
                    try:
                        ...
                        print(f"[DEBUG] Disconnected connection {conn_id} for inactive user {user_id}")
                    except Exception as e:
...
async def submit_task(
    ...
    """
    Submit task with AEA orchestration.
    REPLACES direct command generation/agent logic with a message to the Orchestrator AEA.
    """
    update_user_activity(user_id)
    
    try:
        # 1. Generate a unique ID for this command
        command_id = str(uuid.uuid4())
        
        # 2. CREATE MESSAGE for the Orchestrator AEA
        ...
        # 3. SEND message to the Orchestrator AEA and await CommandPlan response
        # NOTE: In a deployed uagents system, you send to the Orchestrator's address.
        print(f"[DEBUG] Sending task message to Orchestrator AEA at {ORCHESTRATOR_AEA_ADDRESS}")
        
        # --- NEW AEA COMMUNICATION LOGIC ---
        
        # The FastAPI Mailbox Client sends the request to the Orchestrator AEA.
        # This replaces the local agent instantiation and generation logic.
        
        # We must use the asynchronous send_and_receive method (conceptual replacement for simulation)
        # Note: MailboxClient.send_and_receive needs the Orchestrator to respond to the mailbox.
        
        # SIMULATION of successful synchronous AEA exchange
        final_response = await mailbox_client.send_and_receive(
            recipient=ORCHESTRATOR_AEA_ADDRESS,
            message=task_msg,
            response_model=CommandPlan,
            timeout=30.0 # Wait up to 30 seconds for the plan
        )

        if not final_response:
            raise Exception("No response received from Orchestrator AEA within timeout.")
            
        final_plan: CommandPlan = final_response.body
        
        if final_plan.error:
            raise HTTPException(status_code=500, detail=f"AEA plan failure: {final_plan.error}")

        # --- END NEW AEA COMMUNICATION LOGIC ---
        
        # 4. PERSIST COMMANDS IN DATABASE (The Orchestrator should have saved this, but we validate/fallback here)
        db = next(get_db())
        db_service = DatabaseService(db)
        
        # Attempt to get the command using the ID the Orchestrator was instructed to use
        command = db_service.get_command(command_id, user_id) 
        
        if not command:
            # Fallback/Create command record if Orchestrator didn't save (or failed to save)
            command = db_service.create_command(
                user_id=user_id,
                connection_id=task_request.connection_id,
                request=task_request.request,
                intent=final_plan.intent,
                action=final_plan.action,
                risk_level=final_plan.risk_level,
                priority=task_request.priority,
                generated_commands=[step.dict() for step in final_plan.command_plan]
            )
        
        # Log the action (already logged by Orchestrator, but logging submission here for safety)
        db_service.log_action(
            user_id=user_id,
            action="submit_task_to_aea",
            details={
                "command_id": command_id,
                "request": task_request.request,
                "total_steps": len(final_plan.command_plan)
            },
            command_id=command_id,
            connection_id=task_request.connection_id,
            success=True
        ...
        # 5. RETURN RESPONSE
        command_steps = [CommandStep(status="pending", **step.dict()) for step in final_plan.command_plan]
        ...
        return TaskResponse(
            ...
            explanation=final_plan.explanation,
        ...
        print(f"[ERROR] AEA submission failed: {str(e)}")
        # Log failure
        try:
            db = next(get_db())
            db_service = DatabaseService(db)
            db_service.log_action(user_id, "submit_task_failed", details={"error": str(e)}, success=False)
            db.close()
        except:
            pass
        raise HTTPException(status_code=500, detail=f"AEA submission failed: {str(e)}")

# ... (The approve_command_step endpoint needs modification to send ExecutionRequest to Orchestrator) ...
...
async def approve_command_step(
    ...
    """
    Approves a step and sends an ExecutionRequest to the Orchestrator AEA.
    REPLACES the direct CommandExecutor call with AEA communication.
    """
    update_user_activity(user_id)
    ...
    # 1. Create step approval record (same as original logic)
    ...
    # 2. If APPROVED, send ExecutionRequest to Orchestrator AEA
    if approval_request.approved:
        
        step_data = command.generated_commands[approval_request.step_index]
        # Ensure step_data is converted to the Pydantic model for the Protocol
        step_model = CommandStepModel(**step_data) 
        
        execution_msg = ExecutionRequest(
            connection_id=command.connection_id,
            step=step_model
        )
        
        # --- NEW AEA COMMUNICATION LOGIC ---
        print(f"[DEBUG] Sending Execution Request for step {approval_request.step_index} to Orchestrator AEA...")
        
        # The FastAPI Mailbox Client sends the execution request to the Orchestrator AEA.
        
        execution_response = await mailbox_client.send_and_receive(
            recipient=ORCHESTRATOR_AEA_ADDRESS,
            message=execution_msg,
            response_model=ExecutionResult,
            timeout=60.0 # Execution may take longer
        )

        if not execution_response:
            raise Exception("No execution result received from Orchestrator AEA within timeout.")

        simulated_execution: ExecutionResult = execution_response.body
        
        if simulated_execution.error:
            raise Exception(f"AEA Execution Error: {simulated_execution.error}")
        
        execution_result_data = simulated_execution.dict()
        
        # --- END NEW AEA COMMUNICATION LOGIC ---
        
        # 3. Update execution results in database (same as original logic, using AEA result)
        ...
        step_result = {
            ...
            "success": simulated_execution.success,
            "status": "success" if simulated_execution.success else "failed",
            "output": simulated_execution.stdout,
            "stderr": simulated_execution.stderr,
            "error": simulated_execution.error,
            "exit_code": simulated_execution.exit_code,
            "execution_time": simulated_execution.execution_time
        }
        existing_results["step_results"].append(step_result)
        
        # Update counters and overall status
        if simulated_execution.success:
        ...
        existing_results["total_execution_time"] = existing_results.get("total_execution_time", 0) + simulated_execution.execution_time
        ...
        db_service.log_action(
            ...
            command_id=command_id,
            connection_id=command.connection_id, # Use command's connection ID
            success=simulated_execution.success
        )
        ...
        db_service.log_action(
            ...
            details={"command_id": command_id, "step_index": approval_request.step_index, "reason": approval_request.reason or "No reason provided"},
            command_id=command_id,
            success=True
        )
    
    # 4. Final status check and return (same as original logic)
    ...
    # Note: Logic to calculate 'approval_status' is kept in the original file (get_command_approval_status)
    # We simulate a call to that endpoint here for the required return structure.
    
    # Returning a simplified status, as the frontend will typically call /approval-status after this.
    ...
    return {
        ...
        "approval_status": {} # Simplified for this demonstration
    }
...
# from uagents import Model, Agent
# from uagents.mailbox import MailboxClient
# from .protocols import TaskMessage, CommandPlan, ExecutionRequest, ExecutionResult
# # --- END NEW IMPORTS ---
